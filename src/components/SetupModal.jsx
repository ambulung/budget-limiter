...
import { storage, db } from '../firebase'; // Make sure to import storage and db
...
const SetupModal = ({ isOpen, onSave, onClose, user, initialSettings }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setBudget(initialSettings.budget.toString());
  ...
  const handleSave = (e) => {
    ...
    onSave({
      ...
      currency: currency,
      appIcon: currentIconUrl, // Pass the current (potentially new) icon URL back
    });
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4">
      <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-2xl w-full max-w-md relative">
        ...
        {/* --- CORRECTED: "X" Close Button (only shows for existing users) --- */}
        {!initialSettings.isNewUser && (
