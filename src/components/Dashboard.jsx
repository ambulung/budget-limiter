...
// SVG Icons (exported for use in Header or move to a separate Icons file)
...
const Dashboard = ({ user, onLogout }) => {
  ...
  const remainingBudget = parseFloat((budget - totalExpenses).toFixed(2));
  ...
  const handleDeleteAllExpenses = async () => {
    ...
    const deletionPromise = Promise.all(
      expenses.map((expense) => deleteDoc(doc(db, 'users', user.uid, 'expenses', expense.id)))
    );
  ...
  const handleAddExpense = async (e) => {
    ...
    if (!newExpenseDesc || isNaN(amount) || amount <= 0)
      ...
      toast((t) => (
        <span className="flex items-center gap-4">
          ...
          <button
            ...
            onClick={() => {
              handleUndoDelete(expenseId, deletedData);
              toast.dismiss(t.id);
            }}
          >
            Undo
  ...
  return (
    <>
      <SetupModal
        ...
        user={user}
        initialSettings={{ appTitle, appIcon, budget, currency, numberFormat, isNewUser, isGuest }}
      />
      <EditExpenseModal
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md mb-8">
              <div className="flex justify-between items-center mb-2">
                <div className="text-2xl font-bold text-gray-800 dark:text-gray-100">
                  ...
                  <span className="text-gray-400 dark:text-gray-500 text-lg">
                    {' '}
                    / {formatMoney(budget, currency, numberFormat)}
                  </span>
                </div>
              </div>
              ...
              <div className="mt-4 border-t border-gray-200 dark:border-gray-700 pt-4 space-y-2">
                <form onSubmit={handleAddToBudget} className="flex items-center gap-3">
                  ...
                  <button
                    type="submit"
                    className="w-24 px-4 py-2 bg-blue-500 text-white font-semibold rounded-lg shadow-sm hover:bg-blue-600 transition-all flex-shrink-0"
                  >
                    Add
                  </button>
                </form>
                <form onSubmit={handleRemoveFromBudget} className="flex items-center gap-3">
                  ...
                  <button
                    type="submit"
                    className="w-24 px-4 py-2 bg-red-500 text-white font-semibold rounded-lg shadow-sm hover:bg-red-600 transition-all flex-shrink-0"
                  >
                    Remove
                  </button>
                </form>
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md">
                ...
                <form onSubmit={handleAddExpense} className="flex flex-col gap-4">
                  ...
                  <button
                    type="submit"
                    className="p-3 bg-green-500 text-white font-bold rounded-lg shadow-md hover:bg-green-600 transition-all"
                  >
                    Add Expense
                  </button>
                </form>
              ...
              <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md">
                ...
                <ul className="space-y-3 h-[400px] overflow-y-auto pr-2">
                  ...
                  {expenses.map((expense) => (
                    <li
                      key={expense.id}
                      className="flex justify-between items-start bg-slate-100 dark:bg-gray-700 p-3 rounded-lg"
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        {expense.notes && (
                          <p className="text-sm italic text-gray-600 dark:text-gray-400 mt-1 break-words">
                            {expense.notes}
                          </p>
                        )}
                        {expense.createdAt && (
